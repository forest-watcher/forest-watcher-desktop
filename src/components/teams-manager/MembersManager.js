...
class MembersManager extends React.Component {
  ...
  getMembers = () => {
    ...
    let members = selectedManagers.map(manager => ({
      memberType: MANAGER,
      id: manager.id || manager,
      email: manager.email
    }));
    members = members.concat(
      selectedConfirmedUsers.map(confirmedUser => ({
        memberType: CONFIRMED_USER,
        id: confirmedUser.id || confirmedUser,
    ...
    return (
      <div className="c-members-manager small-12 medium-offset-2 medium-5 columns">
        <div className="input-group">
          <h3>
            <label htmlFor="add-member" className="text">
          ...
          <div className="horizontal-field-left-aligned">
            ...
            />
            <button type="button" onClick={this.handleAddEmail} className="c-button -light">
          ...
          <span className="members-tip">
            *members must have a
            <a className="text -green" href={MY_GFW_LINK} target="_blank" rel="noopener noreferrer">
              MyGFW
            </a>
