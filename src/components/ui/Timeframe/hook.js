...
function useTimeline(ref, tileRefs, data, selectedIndex, dotSize, onChange) {
  ...
  useEffect(() => {
    if (ref.current && data && Array.isArray(data) && tileRefs.current && tileRefs.current.length === data.length) {
  ...
  useEffect(() => {
    if (!initialized && timeline.tileWidth && timeline.dataWidth) {
      const timeout = setTimeout(() => {
        const initialOffset = handleOffset(selectedIndex, timeline.tileWidth, data.length, ref, tileRefs);
  ...
  useEffect(() => {
    if (!prevSelectedIndex.current || (selectedIndex !== prevSelectedIndex.current && !isManualInput.current)) {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (ref.current) {
        const visibleLabels = getVisibleLabels(ref.current.getBoundingClientRect(), tileRefs.current);
