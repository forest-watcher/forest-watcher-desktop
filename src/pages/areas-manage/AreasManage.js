...
class AreasManage extends Component {
  ...
  onSubmit = e => {
    ...
    if (this.form.geojson && this.form.name !== "") {
      ...
      if (checkArea(this.form.geojson)) {
        ...
        toastr.error(
          this.props.intl.formatMessage({ id: "areas.tooLarge" }),
          this.props.intl.formatMessage({ id: "areas.tooLargeDesc" })
        );
        ReactGA.event({
          category: CATEGORY.AREA_CREATION,
          action: ACTION.AREA_SAVE,
      ...
      toastr.error(
        this.props.intl.formatMessage({ id: "areas.missingValues" }),
        this.props.intl.formatMessage({ id: "areas.missingValuesDesc" })
      );
      ReactGA.event({
        category: CATEGORY.AREA_CREATION,
        action: ACTION.AREA_SAVE,
    ...
    if (e.target.files[0].size <= maxFileSize) {
      ...
      if (geojson && geojson.features) {
        ...
        if (!checkArea(geojsonParsed)) {
          toastr.error(
            this.props.intl.formatMessage({ id: "areas.tooLarge" }),
            this.props.intl.formatMessage({ id: "areas.uploadedTooLargeDesc" })
          );
          ReactGA.event({
            category: CATEGORY.AREA_CREATION,
            action: ACTION.UPLOAD_SHAPEFILE,
          ...
          if (geojsonParsed) {
            ...
            const dotIndex =
              shapeFile.name.lastIndexOf(".") > -1 ? shapeFile.name.lastIndexOf(".") : shapeFile.name.length;
      ...
      toastr.error(
        this.props.intl.formatMessage({ id: "areas.fileTooLarge" }),
        this.props.intl.formatMessage({ id: "areas.fileTooLargeDesc" })
      );
      ReactGA.event({
        category: CATEGORY.AREA_CREATION,
        action: ACTION.UPLOAD_SHAPEFILE,
  ...
  render() {
    return (
      <div>
        <Hero title={this.props.mode === "manage" ? "areas.manageArea" : "areas.createArea"} />
        ...
        <Form onSubmit={this.onSubmit}>
          <div className="l-map">
            ...
            <div className="c-map-controls">
              <LayersSelector map={this.state.map} />
              <LocationSearch
                intl={this.props.intl}
                onLocationChanged={location => {
                  this.state.map.fitBounds([
                    [location.geometry.viewport.getSouthWest().lat(), location.geometry.viewport.getSouthWest().lng()],
                    [location.geometry.viewport.getNorthEast().lat(), location.geometry.viewport.getNorthEast().lng()]
                  ]);
                }}
          ...
          <div className="row columns">
            <div className="c-form -nav">
              <Link to="/areas">
                <button className="c-button -light" disabled={this.props.saving || this.props.loading}>
                  <FormattedMessage id="forms.cancel" />
                </button>
              </Link>
              <div className="horizontal-field">
                <label className="c-button -light" htmlFor="shapefile">
                  <FormattedMessage id="areas.uploadShapefile" />
                </label>
                <input
                  type="file"
                  id="shapefile"
              ...
              <div className="areas-inputs u-p-relative">
                <div className="horizontal-field">
                  <label className="text -x-small-title">
                    <FormattedMessage id="areas.nameArea" />:{" "}
                  </label>
                  <Input
                    type="text"
              ...
              <Button className="c-button" disabled={this.props.saving || !!this.props.editing || this.props.loading}>
